...
const leafs = input['leafs'].map((bytes32: string) => [bytes32, one_word]);
...
process.stdout.write(ethers.AbiCoder.defaultAbiCoder().encode(['bytes32[]'], [proof]));process.stdout.write(ethers.AbiCoder.defaultAbiCoder().encode(['bytes32[]'], [proof]));